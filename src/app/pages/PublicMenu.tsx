...
export function PublicMenu() {
  ...
  // Set default active menu based on context directly during render or local state initialized dynamically
  ...
  if (error || !business) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center p-8">
        ...
        <p className="text-slate-500">{error || "We couldn't find the menu you're looking for."}</p>
      </div>
    </div>
  );
            ...
            <div className="overflow-x-auto flex gap-2 px-4 py-3 bg-white border-b border-slate-200 hide-scrollbar">
              {uploadedMenus.map(m => (
                <button key={m.id} onClick={() => setSelectedMenuId(m.id)}
                  style={activeMenu?.id === m.id ? { backgroundColor: business.primary_color || '#0f172a', color: '#fff' } : undefined}
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all
                    ${activeMenu?.id === m.id ? "shadow-md" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
            ...
            <div className="overflow-x-auto hide-scrollbar px-4 pb-4 flex gap-2 snap-x">
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  style={activeCategory === cat ? { backgroundColor: business.primary_color || '#0f172a', color: '#fff' } : undefined}
                  className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-all snap-start
                    ${activeCategory === cat ? "shadow-md" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                  {cat}
                </button>
              ))}
            </div>
